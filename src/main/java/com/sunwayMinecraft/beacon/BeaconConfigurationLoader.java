...
/**
 * This class implements the ConfigurationLoader interface and is responsible
 * for loading and managing beacon-related configurations for a Minecraft
 * plugin. It retrieves configuration data from the plugin's config files
 * and processes it accordingly, including the beacon colors and their
 * locations.
 *
 * The class provides methods to:
 * - Retrieve the number of ticks per transition for beacons.
 * - Load a list of beacon colors from the configuration file and validate
 *   them.
 * - Load beacon locations from a separate YAML configuration file and
 *   store them in a map.
 *
 * The class ensures proper validation of configuration values, logging
 * warnings for invalid entries and defaults when necessary. If no valid
 * beacon colors are found in the configuration, the default color is
 * set to MAGENTA_STAINED_GLASS.
 *
 * The methods provided are as follows:
 * - `getTicksPerTransition()`: Retrieves the number of ticks per
 *   transition for beacons from the plugin's config.
 * - `getColorCycle()`: Loads and validates a list of beacon colors
 *   from the configuration, defaulting to MAGENTA_STAINED_GLASS if
 *   none are valid.
 * - `loadBeacons()`: Loads beacon locations from a YAML file and
 *   returns a map of location objects to their corresponding color
 *   indices.
 */
