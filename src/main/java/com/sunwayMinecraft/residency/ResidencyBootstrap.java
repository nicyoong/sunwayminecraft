...
public class ResidencyBootstrap {
    ...
    public ResidencyBootstrap(JavaPlugin plugin, Economy economy) {
        ...
        ResidencyManager temp = new ResidencyManager(plugin, districts, buildings, units, pricing, policy, repository, null);
        BillingService billingService = new BillingService(temp, economy);
        this.manager = new ResidencyManager(plugin, districts, buildings, units, pricing, policy, repository, billingService);
    }
    ...
    private void register(String name, CommandExecutor executor) {
        if (plugin.getCommand(name) != null) plugin.getCommand(name).setExecutor(executor);
        else plugin.getLogger().warning("Command '" + name + "' not found in plugin.yml!");
    }
}
