...
/**
 * The LightManager class provides utilities for managing “light” blocks within configured regions.
 * It uses mappings between light-emitting materials and their non-light counterparts to identify,
 * scan, and convert blocks in a region.
 *
 * <p>Key functionality includes:
 *
 * <ul>
 *   <li>{@link #scanRegion(LightRegion, Player)}: Scans a region for blocks whose material is
 *       defined in the light mappings.
 *   <li>{@link #isLightBlock(Material)}: Checks if a given material is a light block.
 *   <li>{@link #getOppositeMaterial(Material)}: Retrieves the paired material (light ↔ non-light)
 *       for a given material.
 *   <li>{@link #getOffMaterial(Material)}: Gets the non-light version of a light block.
 *   <li>{@link #getOriginalMaterial(Material)}: Gets the light-emitting version for a non-light
 *       block.
 * </ul>
 *
 * <p>Usage example:
...
public class LightManager {
  private static final Map<Material, Material> LIGHT_MAPPINGS =
      Map.of(
  ...
  public static Material getOppositeMaterial(Material material) {
    return LIGHT_MAPPINGS.getOrDefault(
        material,
   ...
   *   <li>Adds matching blocks to the result list.
   * </ol>
   *
   * <p>This method is typically used to locate all light-emitting blocks within a region so that
   * they can be toggled or processed further.
   *
