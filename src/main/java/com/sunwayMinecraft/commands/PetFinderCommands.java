...
/**
 * The {@code PetFinderCommands} class handles commands related to finding pets in the Minecraft
 * plugin. It processes two main commands:
 *
 * <ul>
 *   <li><b>/findpets</b> - Searches for pets near a specified or invoking player.
 *   <li><b>/findpetsinarea</b> - Searches for pets within a user-defined bounding box (admin only),
 *       optionally targeting a specific player.
 * </ul>
 *
 * <p>Key functionality includes:
 *
 * <ul>
 *   <li>Permission checks to ensure the sender has the required {@code petfinder.admin} permission
 *       for targeting other players or performing area searches.
 *   <li>Parsing of player targets, either from command arguments or defaulting to the invoking
 *       player.
 *   <li>Parsing of a 3D bounding box from six coordinate arguments, normalizing minima and maxima.
 *   <li>Delegating search logic to the {@link PetFinderManager}.
 * </ul>
 *
 * <p>Commands are routed based on the command name, with each handler method providing clear usage
 * messages, permission checks, and error handling.
 */
public class PetFinderCommands implements CommandExecutor {
  ...
  /**
   * Handles incoming commands registered to this executor. Supported commands are {@code findpets}
   * and {@code findpetsinarea}.
   *
   * <p>The method routes the command based on its name, invoking the appropriate handler. If the
   * command matches one of the supported names, it returns {@code true}, indicating that the
   ...
   *
   * <ol>
   *   <li>Attempts to resolve the target player's UUID via {@link #parseTargetUUID}.
   *   <li>If resolution fails (permissions or player not found), the method returns early.
   *   <li>Invokes {@link PetFinderManager#startSearch} with the sender, target UUID, and no area.
   * </ol>
   *
   * <p>This command can target another player only if the sender has {@code petfinder.admin}
   * permission; otherwise, it defaults to the invoking player.
   *
  ...
  /**
   * Handles the {@code /findpetsinarea} command, which searches for pets within a specified
   * bounding box.
   *
   * <p>The method performs the following steps:
   *
   * <ol>
   *   <li>Checks that the sender has {@code petfinder.admin} permission; if not, sends an error
   *       message.
   *   <li>Verifies that at least six coordinate arguments are provided; if not, sends usage
   *       instructions.
   *   <li>Parses the six coordinates into a {@link BoundingBox} via {@link #parseBoundingBox}.
   *   <li>Optionally resolves a target player UUID from additional args (admin only).
   *   <li>If running from console without a target player, prompts to specify a player.
   *   <li>Invokes {@link PetFinderManager#startSearch} with the sender, target UUID, and area.
   * </ol>
   *
  ...
  private void handleAreaSearch(CommandSender sender, String[] args) {
    ...
    try {
      ...
      UUID targetUUID =
          args.length > 6
              ? parseTargetUUID(sender, Arrays.copyOfRange(args, 6, args.length))
              : null;
   ...
   *
   * <p>The method expects coordinates in the order: {@code x1, y1, z1, x2, y2, z2}. It normalizes
   * the values so that minima and maxima are assigned correctly.
   *
   ...
   *
   * <ol>
   *   <li>If {@code args} contains a player name and the sender has {@code petfinder.admin}
   *       permission, attempts to locate the online player by name.
   *   <li>If the player is not found or the sender lacks permission, sends an error message.
   *   <li>If no arguments are provided and the sender is a player, returns the sender's UUID.
   *   <li>If no arguments and the sender is console, prompts to specify a player.
   * </ol>
   *
   * @param sender the source of the command (player or console)
