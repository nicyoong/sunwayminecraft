...
/**
 * The {@code PetSearchTask} class performs a batched search for tamed pets (dogs and cats) within
 * the Minecraft world, optionally filtering by owner and bounding area.
 *
 * <p>This task is scheduled via the Bukkit scheduler and processes entities in configurable batch
 * sizes to avoid blocking the main server thread.
 *
 * <p>Key responsibilities include:
 *
 * <ul>
 *   <li>Filtering entities to valid, tamed pets owned by a specific player (or all owners if {@code
 *       targetUUID} is null).
 *   <li>Restricting results to those within an optional {@link BoundingBox} area.
 *   <li>Batch-processing a list of entities each run tick to spread workload.
 *   <li>Tracking and logging chunk progress every ten unique chunks scanned.
 *   <li>Aggregating and sending detailed search results, including pet counts and statuses, to the
 *       command sender upon completion.
 * </ul>
 *
 * @see BukkitRunnable
 */
public class PetSearchTask extends BukkitRunnable {
   ...
   */
  public PetSearchTask(
      JavaPlugin plugin,
      CommandSender sender,
      List<Entity> entities,
      UUID targetUUID,
      BoundingBox area,
      PetFinderManager manager,
      int totalChunks) {
    this.plugin = plugin;
    this.sender = sender;
   ...
   *
   * <ol>
   *   <li>Processes up to {@code batchSize} entities from the list.
   *   <li>Filters out invalid entities, untamed pets, and those outside the search area.
   *   <li>Invokes {@link #addToResults(Entity)} for each valid pet to record details.
   *   <li>When the list is exhausted, sends final results via {@link #sendFinalResults()} and marks
   *       the search complete.
   *   <li>Tracks the current chunk of the next entity, updating {@code processedChunks} and {@code
   *       lastChunks}.
   *   <li>Logs progress every ten unique chunks scanned to the plugin logger.
   * </ol>
   *
   * <p>This method runs on the server scheduler thread and must complete quickly to avoid tick lag.
   */
  @Override
  public void run() {
    int batchSize = 50;
  ...
  /**
   * Adds a detailed description of a valid pet to the results list, including type, custom name (if
   * any), current and max health, world coordinates, and sitting state. Also increments the
   * appropriate count.
   *
   * <p>This method performs:
   *
   * <ol>
   *   <li>Determines pet type ({@code Dog} or {@code Cat}) and increments counters.
   *   <li>Extracts and formats a custom name, defaulting to "Unnamed".
   *   <li>Retrieves and rounds current and maximum health values.
   *   <li>Checks if the pet is sitting via {@link Sittable}.
   *   <li>Formats the combined details into a string and appends to {@code results}.
   * </ol>
   *
   ...
   *
   * <ol>
   *   <li>If no dogs or cats were found, sends a "No matching pets found" message.
   *   <li>Otherwise, sends a summary count message indicating total pets, number of dogs, and
   *       number of cats.
   *   <li>Sends each formatted pet detail line from {@code results}.
   * </ol>
   *
